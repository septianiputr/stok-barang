...
<div class="container">
    <div class="p-4 mx-auto col-6 bg-white">
        ...
        <div class="mb-4">
            <h2 class="fw-bold">Tambah Stok Barang Setengah Jadi</h2>
        </div>

        <form action="{{ route('barang-setengah-jadi-masuk.tambah') }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <select name="kode_barang" id="kode_barang" class="form-control" required>
                    @foreach ($barangSetengahJadi as $barang_setengah_jadi)
                    <option value="{{ $barang_setengah_jadi->id }}" data-nama="{{ $barang_setengah_jadi->nama }}">
                        {{ $barang_setengah_jadi->nama }} - {{ $barang_setengah_jadi->id }}
                    </option>
                    @endforeach
                </select>
            </div>
            ...
            <a href="{{ route('barang-setengah-jadi-masuk.show') }}" class="btn btn-secondary mt-3">Kembali</a>
        </form>
    </div>
</div>
